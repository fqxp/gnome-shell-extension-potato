  ...
  class PotatoIndicator extends PanelMenu.Button {
    ...
    _buildMenu() {
      ...
      this._pauseItem.connect("activate", () => {
        if (this._timer.isPaused) this._timer.resume();
        else this._timer.pause();
      });
    ...
    _startBlinking() {
      ...
      this._blinkSourceId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 600, () => {
        ...
        if (this._label) this._label.opacity = this._blinkVisible ? 255 : 80;
    ...
    _stopBlinking() {
      ...
      if (this._label) this._label.opacity = 255;
    }

    destroy() {
      this._stopBlinking();

      for (const id of this._timerSignals) this._timer.disconnect(id);
      this._timerSignals = [];
